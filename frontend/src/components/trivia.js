...
const Trivia = () => {
  ...
  const [questions, setQuestions] = useState(
    originalQuestions.map((q) => ({ ...q, options: shuffleArray(q.options) }))
  );
  ...
  const handleAnswerClick = (answer) => {
    ...
    if (answer === correctAnswer) {
      ...
      setFeedback(
        `❌ You'll get it next time! The correct answer is: "${correctAnswer}"`
      );
    }
  };
  ...
  if (isGameOver) {
    return (
      <div className={styles.triviaContainer}>
        ...
        <button className={styles.nextButton} onClick={restartGame}>
          Play Again
        </button>
      </div>
    );
  ...
  return (
    <div className={styles.triviaContainer}>
      <nav className={styles.navbar}>
        ...
        <button className={styles.navButton} onClick={() => navigate("/games")}>
          🎮 Games Menu
        </button>
      </nav>
      <h1>Bible Trivia</h1>
